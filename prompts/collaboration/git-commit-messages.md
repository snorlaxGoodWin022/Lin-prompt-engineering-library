...
| 参数 | 说明 | 示例 |
|------|------|------|
| `{简单描述改动或贴git diff}` | 代码改动描述或 diff | `修改了UserList翻页逻辑` |
...
1. 可以直接粘贴 `git diff` 输出，AI 会自动分析
2. 也可以用白话描述改了什么，AI 会转化为规范格式
3. 3个选项从不同角度描述改动，选择最符合意图的即可
4. type 选择：修bug用fix，新功能用feat，重构用refactor